...
namespace FleshHive.Odyssey;

public class SectionLayer_FleshTerrainEdges : SectionLayer
{
	public SectionLayer_FleshTerrainEdges(Section section)
		: base(section)
	{
	...
	public override void Regenerate()
	{
		...
		if (!ModsConfig.OdysseyActive || !base.Map.Tile.Valid || !base.Map.Tile.LayerDef.isSpace)
		{
			return;
		}
		TerrainGrid terrainGrid = base.Map.terrainGrid;
		CellRect cellRect = section.CellRect;
		float altitude = AltitudeLayer.TerrainScatter.AltitudeFor() + 0.001f;
		float altitude2 = AltitudeLayer.TerrainEdges.AltitudeFor() + 0.001f;
		foreach (IntVec3 item in cellRect)
		{
			if (ShouldDrawRockEdges(item, terrainGrid, out var edges, out var corners))
			{
				TerrainDef terrain = GetEdgeTerrain(item, terrainGrid);
	...
	private void DrawLoop(IntVec3 c, TerrainGrid grid, EdgeDirections edges, CornerDirections corners, float altitude)
	{
		...
		{
			TerrainDef terrain = GetEdgeTerrain(c + IntVec3.North, grid);
	...
	private bool ShouldDrawRockEdges(IntVec3 c, TerrainGrid grid, out EdgeDirections edges, out CornerDirections corners)
	{
		...
		TerrainDef terrainDef = GetEdgeTerrain(c, grid);
		if (!IsFleshTerrain(grid.TerrainAt(c)) || grid.BaseTerrainAt(c) == terrainDef || terrainDef.spaceEdgeGraphicData == null)
		{
			return false;
		}
	...
	private bool ShouldDrawPassthrough(IntVec3 c, TerrainGrid grid, out EdgeDirections edges, out CornerDirections corners)
	{
		...
		TerrainDef terrainDef = GetEdgeTerrain(c2, grid);
	...
	private bool ShouldDrawLoop(IntVec3 c, TerrainGrid grid, out EdgeDirections edges, out CornerDirections corners)
	{
		...
		TerrainDef terrainDef2 = GetEdgeTerrain(c2, grid);
		if (!IsFleshTerrain(grid.TerrainAt(c2)) || grid.BaseTerrainAt(c2) == terrainDef2 || terrainDef2.spaceEdgeGraphicData == null)
		{
			return false;
		}
		for (int i = 0; i < GenAdj.CardinalDirections.Length; i++)
	...
	private static bool IsFleshTerrain(TerrainDef? terrain)
	{
		return terrain != null && (terrain == TerrainDefOf.Flesh || terrain.defName == "FH_FleshCarapaceFloor");
	}

	private static TerrainDef GetEdgeTerrain(IntVec3 cell, TerrainGrid grid)
	{
		TerrainDef terrain = grid.TerrainAt(cell);
		return IsFleshTerrain(terrain) ? terrain : grid.BaseTerrainAt(cell);
	}
